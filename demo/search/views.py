...
attr_map = build_dict.load_attr_map("./search/data/attr_mapping.txt")
# attr_ac = cPickle.load(open("./search/data/attr_ac.pkl", "rb"), encoding="bytes")
attr_ac = build_dict.dump_ac_attr_dict(build_dict.attr_list_file, build_dict.attr_out_path)
attr_ac.make_automaton()
ent_dict = build_dict.load_entity_dict("./search/data/all_entity.txt")
val_dict = build_dict.load_val_dict("./search/data/Person_val.txt")
...
def _search_single_subj(entity_name):
    ...
    response = requests.post("http://localhost:9200/demo/person/_search?&pretty", data=query, headers=headers)
    res = json.loads(response.content)
        ...
        for a in retain_attr:
            ...
            if mapped_a == 'height':
                ...
                if re.search('大于等于|不小于|>=|不矮于', between):
                    ...
                    part_query = "身高<=" + height
                elif re.search('大于|高于|超过|>', between) or (between == '比' and nl_query[height_pos+len(height)] in
                    ['高', '大']):
                    part_query = "身高>" + height
                elif re.search('小于|矮于|<', between) or (between == '比' and nl_query[height_pos+len(height)] in
                    ['矮', '小']):
                    part_query = "身高<" + height
